...
namespace LA {
  ...
  void BBGenerator::visit(Instruction_declaration *i) {
    ...
    */
    if (i->get_type()->to_string() == "int64") {
      new_instructions.push_back(new Instruction_assignment(i->get_var(), new Number(1)));
    } else {
      new_instructions.push_back(new Instruction_assignment(i->get_var(), new Number(0)));
    }
