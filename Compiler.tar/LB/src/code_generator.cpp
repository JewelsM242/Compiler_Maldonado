  ...
  void generate_code(Program p){
    ...
    for (auto f : p.functions) {
      for (auto i : f->instructions) {
         i->printInstruction();
         if(dynamic_cast<Instruction_assignment*>(i)!=nullptr){
          printg("Is Assignment");
         }
      }
    }
    printg("\n-End Full Read-");
    ...
    for(int i=0;i<currentScopeId;i++){
      ...
      myScope=s;
      printg(to_string(s->contentsOfScope.size()));
